...
function onClientRender()
	...
	-- Draw title
	dxDrawText("Konsola Pomocy", x + 3, y + 1, x + height + 3, y + height + 1, black, 1.0, "default-bold", "center", "top", true, false, true, false, true, 270)
	dxDrawText("Konsola Pomocy", x + 2, y, x + height + 2, y + height, title_color, 1.0, "default-bold", "center", "top", true, false, true, false, true, 270)
	...
	if (active and tabdata[active]) then
		...
		for i = 1, (linecount) do
			...
			dxDrawText(index, x + 25, h, x + 25 + 45, h + lineheight, data.color, 1.0, "default-bold", "left", "center", true, false, true, false, true)
...
function onClientDebugMessage(message, level, from)
...
addEventHandler("onClientResourceStart", resourceRoot,
	function ()
		-- Standard types
		createDebugTab("CZAT", tocolor(0, 255, 0), "CZAT")
		createDebugTab("PMY", tocolor(0, 0, 180), "PMY")
		createDebugTab("RAPORTY", tocolor(255, 0, 0), "RAPORTY")
		createDebugTab("PRZELEWY", tocolor(0, 150, 80), "PRZELEWY")
...
addCommandHandler("qkx.konsola", function(cmd)
...
function outputDebugTab(name, message, from)
	...
	table.insert(tabdata[name].messages, {text = message, from = from, stamp = stamp, count = tabdata[name].count})
end
