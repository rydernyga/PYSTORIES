...
od.lbl_oplacanie = guiCreateLabel(0.026,0.0686,0.9524,0.2455,"Koszt wynajęcia domu na okres x dni wynosi yPLN", true, od.tab_oplacanie)
...
od.lbl4 = guiCreateLabel(0.026,0.5271,0.6688,0.1336,"Koszt : 5555",true,od.tab_oplacanie)
...
od.btn_bezdmon = guiCreateButton(0.026,0.4007,0.4307,0.2202,"Zwolnij dom ",true,od.tab_opcje)
...
--	toggleControl("fire", false)
	...
	if dom.ownerid then
		...
		if dbid and dom.ownerid==dbid then
			...
			guiSetEnabled(od.btn_bezdmon,true)
	...
	local reklama=string.format("Zaoszczędź %sPLN i opłać dom na 30 dni za pomocą jednego SMSa.", ((dom.koszt/100)*2)*30)
--	local reklama=""
...
--	if getPlayerName(localPlayer)=="ZiomzOsia" then
		a_dom=dom
		pokazOknoDomu(dom)
--	end
...
addEventHandler("onClientGUIClick", od.btn_bezdmon, function()
	if not a_dom then return end
