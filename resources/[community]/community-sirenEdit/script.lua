...
--local DEF_KeyBind = "0"
--local DEF_Command = "qkx.esiren-bone"
...
addCommandHandler("qkx.esiren-bone", sirenCmd)
...
function InitUI ( )
	...
	if ( veh ) then
		...
		guiSetText ( posLabelX, "Pozycja X: " .. string.format( "%.3f", sirensTable[selectedItem].x ) )
		guiSetText ( posLabelY, "Pozycja Y: " .. string.format( "%.3f", sirensTable[selectedItem].y ) )
		guiSetText ( posLabelZ, "Pozycja Z: " .. string.format( "%.3f", sirensTable[selectedItem].z ) )
		...
		guiSetText ( sirensBtnRGB, string.format ( "Kolor: #%02X%02X%02X", sirensTable[selectedItem].Red, sirensTable[selectedItem].Green, sirensTable[selectedItem].Blue ) )
...
function btnFunc ( button, state )
	...
	if ( veh ~= false ) then
		...
		if ( source == sirensBtnGet ) then
			...
			outputChatBox("setVehicleSirens ( vehicle, " ..selectedItem .. ", " .. string.format( "%.3f", v.x ) .. ", " .. string.format( "%.3f", v.y ) .. ", " .. string.format( "%.3f", v.z ) .. ", " .. v.Red .. ", " .. v.Green .. ", " .. v.Blue .. ", " .. v.Alpha .. ", " .. v.Min_Alpha .. " )" )
			outputChatBox("addVehicleSirens ( vehicle, " .. vParamsTable.SirenCount .. ", " .. vParamsTable.SirenType .. ", " .. tostring ( vParamsTable.Flags["360"] ) .. ", " .. tostring ( vParamsTable.Flags.DoLOSCheck ) .. ", " .. tostring ( vParamsTable.Flags.UseRandomiser ) .. ", " .. tostring ( vParamsTable.Flags.Silent ) .. " ) "  )
		end
...
function test ( button, state, l, f, x, y, z, element )
	...
	if ( element == veh and veh ~= false and bSelecting == true ) then	
		...
		guiSetText( posLabelX, "Poyzcja X: " .. string.format( "%.3f", sirensTable[selectedItem].x ) )

		sirensTable[selectedItem].y = fixOutput ( result[2] )
		guiSetText( posLabelY, "Poyzcja Y: " .. string.format( "%.3f", sirensTable[selectedItem].y ) )
