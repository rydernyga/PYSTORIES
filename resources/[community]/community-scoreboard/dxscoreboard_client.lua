	...
	if #scoreboardColumns ~= 0 then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.textFunction then
							if content and column.name == "name" and type( content ) == "table" then
								...
								content[1] = column.textFunction( content[1], nil )
								end
							else
								if not getElementData( player, "p:inv" ) then 
								content = column.textFunction( content, player )
								else
								content = column.textFunction( content, nil )
								end
							end
						end
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				dxDrawText("Forum: mta-pystories.xaa.pl", topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(1.00)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
			
			local y = topY+s(5)
			dxDrawImage( topX-s(10), y-30, 200, 33, "logo.png", 0,0,0, cWhite, drawOverGUI )
			
			
			if serverInfo.players and showServerInfo then
				local players = getPlayers()--getElementsByType( "player" )
				local text = "Aktywni: " .. tostring( players ) .. "/" .. serverInfo.players
			...
			if serverInfo.map and showGamemodeInfo then
				local text = "Mapa: " .. serverInfo.map
			...
			dxDrawText( "Nacisnij prawy przycisk myszy by scrolować", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
...
local i = 0
	...
	if not getElementData(thePlayer,"p:inv") then
	i = i+1
	end
	end
return i
